...
if ($UploadArtifacts) {
    ...
    if ($null -ne ($JsonParameters | Get-Member -Type NoteProperty 'parameters')) {
        ...
        foreach($param in $JsonParameters.psobject.Properties) {
            $AdditionalParameters.Add($param.Name, $param.Value.value)
        }

    }
    $ArtifactsLocationName = '_artifactsLocation'
    $ArtifactsLocationSasTokenName = '_artifactsLocationSasToken'
    $AdditionalParameters[$ArtifactsLocationName] = $JsonParameters | Select -Expand $ArtifactsLocationName -ErrorAction Ignore | Select -Expand 'value' -ErrorAction Ignore
    #$AdditionalParameters[$ArtifactsLocationSasTokenName] = $JsonParameters | Select -Expand $ArtifactsLocationSasTokenName -ErrorAction Ignore | Select -Expand 'value' -ErrorAction Ignore
    ...
    if ($null -eq $AdditionalParameters[$ArtifactsLocationName]) {
        $AdditionalParameters[$ArtifactsLocationName] = $StorageAccount.Context.BlobEndPoint + $StorageContainerName
    }
    ...
    if ($null -eq $AdditionalParameters[$ArtifactsLocationSasTokenName]) {
        $AdditionalParameters[$ArtifactsLocationSasTokenName] = ConvertTo-SecureString -AsPlainText -Force `
